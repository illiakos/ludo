...
ClickHandler::ClickHandler(GLFWwindow *window)
    : window(window), cursorX(0), cursorY(0) {
  instances[window] = this;
  glfwSetMouseButtonCallback(window, mouseButtonCallback);
  glfwSetCursorPosCallback(window, cursorPositionCallback);
}

ClickHandler::~ClickHandler() { instances.erase(window); }

void ClickHandler::registerClickCallback(int button, ClickCallback callback) {
  callbacks[button] = callback;
}

void ClickHandler::mouseButtonCallback(GLFWwindow *window, int button,
                                       int action, int mods) {
...
void ClickHandler::cursorPositionCallback(GLFWwindow *window, double xpos,
                                          double ypos) {
  auto it = instances.find(window);
  if (it != instances.end()) {
