...
void MapDrawer::drawRectangle (float x,
    float y,
    float width,
    float height,
...
void MapDrawer::drawCircle (
    float cx, float cy, float radius, const Color &color, int segments = 1000) {
...
void MapDrawer::drawTriangle (
    float x1, float y1, float x2, float y2, float x3, float y3, const Color &color) {
  glColor3f (color.getRedf (), color.getGreenf (), color.getBluef ()); // Set color
...
void MapDrawer::drawStar (
    float cx, float cy, float outerRadius, float innerRadius, int numPoints, const Color color) {
  if (numPoints < 5) {
...
void MapDrawer::drawArrow (float xStart,
    float yStart,
    float xEnd,
    float yEnd,
    float headLength,
...
void MapDrawer::drawBase (float x, float y, const Color &color) {
  ...
  // Draw colored bg
  drawRectangle (getCellPosition (x),
      getCellPosition (y),
      getSizeOfCells (6),
      getSizeOfCells (6),
      color); // Blue color
  ...
  // Draw white part
  drawRectangle (getCellPosition (x + 1),
      getCellPosition (y + 1),
      getSizeOfCells (4),
      getSizeOfCells (4),
      white); // White color
...
void MapDrawer::drawRoads () {
  ...
  // Draw blue road
  drawRectangle (
      getCellPosition (6), getCellPosition (1), getSizeOfCells (1), getSizeOfCells (1), blue);
  drawRectangle (
      getCellPosition (7), getCellPosition (1), getSizeOfCells (1), getSizeOfCells (5), blue);

  // Draw red road
  drawRectangle (
      getCellPosition (1), getCellPosition (8), getSizeOfCells (1), getSizeOfCells (1), red);
  drawRectangle (
      getCellPosition (1), getCellPosition (7), getSizeOfCells (5), getSizeOfCells (1), red);

  // Draw green road
  drawRectangle (
      getCellPosition (8), getCellPosition (13), getSizeOfCells (1), getSizeOfCells (1), green);
  drawRectangle (
      getCellPosition (7), getCellPosition (9), getSizeOfCells (1), getSizeOfCells (5), green);

  // Draw yellow road
  drawRectangle (
      getCellPosition (13), getCellPosition (6), getSizeOfCells (1), getSizeOfCells (1), yellow);
  drawRectangle (
      getCellPosition (9), getCellPosition (7), getSizeOfCells (5), getSizeOfCells (1), yellow);
}
...
void MapDrawer::drawCells () {
  ...
  for (int i = 0; i < mapSize; i++) {
    for (int j = 0; j < mapSize; j++) {
      drawRectangle (getCellPosition (i),
          getCellPosition (j),
          sizeOfCell,
          sizeOfCell,
          white,
...
void MapDrawer::drawMiddle () {
  ...
  drawRectangle (
      getCellPosition (6), getCellPosition (6), getSizeOfCells (3), getSizeOfCells (3), gray);
...
void MapDrawer::removeRenderable (std::shared_ptr<Renderable> renderable) {
  std::lock_guard<std::mutex> lock (renderablesMutex);
  renderableItems.erase (std::remove (renderableItems.begin (), renderableItems.end (), renderable),
      renderableItems.end ());
}
