...
void Pawn::renderSelf() const {
  ...
  if (context == TileContext::Walkable) {
    ...
    for (int i = 0; i <= segments; i++) {
      ...
      glVertex2f(dimensions.x + x + halfCellSize,
                 dimensions.y + y + halfCellSize);
    }
    ...
    drawer.drawCircle(drawer.getCellPosition(currentBase->getDimensions().x + inBaseCoords.first), drawer.getCellPosition(getDimensions().y + inBaseCoords.second), drawer.getSizeOfCells(0.45), red, 10000);  
