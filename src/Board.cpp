...
// Add finishing tile
void Board::addFinishingTile (
    Dimensions d, int id, int position, int indexInPath, int pathLength, Color &color) {
  tiles.push_back (
      std::make_unique<FinishingTile> (d, id, position, color, indexInPath == (pathLength - 1)));
}
...
void Board::printBoard () const {
  for (const auto &tile : tiles) {
    ...
    std::cout << "Tile ID: " << tile->getId () << " at (" << coords.x << ", " << coords.y
