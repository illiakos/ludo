...
sf::Color colorFromFloats (float r, float g, float b) {
  return sf::Color (
      static_cast<sf::Uint8> (r), static_cast<sf::Uint8> (g), static_cast<sf::Uint8> (b));
}
...
sf::Color getTerrainColor (float elevation, float moisture) {
  sf::Color color =
      elevation < 0.11f
          ? sf::Color (0, 0, static_cast<sf::Uint8> (elevation / 0.11f * 74.f + 181.0f))
      : elevation < 0.14f
    ...
    for (unsigned int x = 0; x < resolutionX; x++) {
      ...
      if (x > 0) {
        ...
        vertices[arrayIndexBase + 0].position = sf::Vector2f (
            static_cast<float> (x) * scalingFactorX, static_cast<float> (y) * scalingFactorY);
      ...
      if (x > 0) {
        ...
        vertices[arrayIndexBase + 1].position = sf::Vector2f (
            static_cast<float> (x) * scalingFactorX, static_cast<float> (y + 1) * scalingFactorY);
      ...
      vertices[arrayIndexBase + 2].position = sf::Vector2f (
          static_cast<float> (x + 1) * scalingFactorX, static_cast<float> (y + 1) * scalingFactorY);
      ...
      if (y > rowStart) {
        ...
        vertices[arrayIndexBase + 5].position = sf::Vector2f (
            static_cast<float> (x + 1) * scalingFactorX, static_cast<float> (y) * scalingFactorY);
