...
void matrixLookAt (Matrix &result, const Vec3 &eye, const Vec3 &center, const Vec3 &up) {
  ...
  result[3][3] = (-eye[0]) * result[0][3] + (-eye[1]) * result[1][3] + (-eye[2]) * result[2][3]
                 + 1.0f;
}
...
public:
  ...
  VulkanExample ()
      : window (sf::VideoMode (800, 600), "SFML window with Vulkan", sf::Style::Default),
        vulkanAvailable (sf::Vulkan::isAvailable ()), maxFramesInFlight (2), currentFrame (0),
        swapchainOutOfDate (false), instance (0), debugReportCallback (0), surface (0), gpu (0),
        queueFamilyIndex (-1), device (0), queue (0), swapchainFormat (), swapchainExtent (),
        swapchain (0), depthFormat (VK_FORMAT_UNDEFINED), depthImage (0), depthImageMemory (0),
        depthImageView (0), vertexShaderModule (0), fragmentShaderModule (0),
        descriptorSetLayout (0), pipelineLayout (0), renderPass (0), graphicsPipeline (0),
        commandPool (0), vertexBuffer (0), vertexBufferMemory (0), indexBuffer (0),
        indexBufferMemory (0), textureImage (0), textureImageMemory (0), textureImageView (0),
  ...
  void setupInstance () {
    ...
    if (result == VK_ERROR_EXTENSION_NOT_PRESENT) {
      ...
      instanceCreateInfo.enabledExtensionCount = static_cast<sf::Uint32> (
          requiredExtentions.size ());
      instanceCreateInfo.ppEnabledExtensionNames = &requiredExtentions[0];
  ...
  void setupDebugReportCallback () {
    ...
    if (vkCreateDebugReportCallbackEXT (
            instance, &debugReportCallbackCreateInfo, 0, &debugReportCallback)
        != VK_SUCCESS) {
      vulkanAvailable = false;
      return;
    }
  ...
  void setupPhysicalDevice () {
    ...
    for (std::size_t i = 0; i < devices.size (); i++) {
      ...
      if (vkEnumerateDeviceExtensionProperties (devices[i], 0, &objectCount, &extensions[0])
          != VK_SUCCESS) {
        vulkanAvailable = false;
        return;
      }
  ...
  void setupLogicalDevice () {
    ...
    for (std::size_t i = 0; i < queueFamilyProperties.size (); i++) {
      ...
      vkGetPhysicalDeviceSurfaceSupportKHR (
          gpu, static_cast<sf::Uint32> (i), surface, &surfaceSupported);
  ...
  void setupSwapchain () {
    ...
    if (vkGetPhysicalDeviceSurfaceFormatsKHR (gpu, surface, &objectCount, &surfaceFormats[0])
        != VK_SUCCESS) {
      vulkanAvailable = false;
      return;
    }
    ...
    if (vkGetPhysicalDeviceSurfacePresentModesKHR (gpu, surface, &objectCount, &presentModes[0])
        != VK_SUCCESS) {
      vulkanAvailable = false;
      return;
    }
    ...
    if (vkGetPhysicalDeviceSurfaceCapabilitiesKHR (gpu, surface, &surfaceCapabilities)
        != VK_SUCCESS) {
      vulkanAvailable = false;
      return;
    }
    ...
    uint32_t imageCount = clamp<uint32_t> (
        2, surfaceCapabilities.minImageCount, surfaceCapabilities.maxImageCount);
  ...
  void setupSwapchainImages () {
    ...
    if (vkGetSwapchainImagesKHR (device, swapchain, &objectCount, &swapchainImages[0])
        != VK_SUCCESS) {
      vulkanAvailable = false;
      return;
    }
    ...
    for (std::size_t i = 0; i < swapchainImages.size (); i++) {
      ...
      if (vkCreateImageView (device, &imageViewCreateInfo, 0, &swapchainImageViews[i])
          != VK_SUCCESS) {
        vulkanAvailable = false;
        return;
      }
  ...
  void setupShaders () {
    ...
    // Use the vertex shader SPIR-V code to create a vertex shader module
    {
      ...
      if (vkCreateShaderModule (device, &shaderModuleCreateInfo, 0, &vertexShaderModule)
          != VK_SUCCESS) {
        vulkanAvailable = false;
        return;
      }
    ...
    // Use the fragment shader SPIR-V code to create a fragment shader module
    {
      ...
      if (vkCreateShaderModule (device, &shaderModuleCreateInfo, 0, &fragmentShaderModule)
          != VK_SUCCESS) {
        vulkanAvailable = false;
        return;
      }
  ...
  void setupPipeline () {
    ...
    // Set up rasterization parameters: fill polygons, no backface culling, front face is
    // counter-clockwise
    ...
    if (vkCreateGraphicsPipelines (
            device, VK_NULL_HANDLE, 1, &graphicsPipelineCreateInfo, 0, &graphicsPipeline)
        != VK_SUCCESS) {
      vulkanAvailable = false;
      return;
    }
  ...
  void setupFramebuffers () {
    ...
    for (std::size_t i = 0; i < swapchainFramebuffers.size (); i++) {
      ...
      if (vkCreateFramebuffer (device, &framebufferCreateInfo, 0, &swapchainFramebuffers[i])
          != VK_SUCCESS) {
        vulkanAvailable = false;
        return;
      }
  ...
  void setupVertexBuffer () {
    float vertexData[] = {// X      Y      Z     R     G     B     A     U     V
        -0.5f,
        -0.5f,
        0.5f,
        1.0f,
  ...
  void setupUniformBuffers () {
    ...
    for (size_t i = 0; i < swapchainImages.size (); i++) {
      ...
      // The uniform buffer will be host visible and coherent since we use it for streaming data
      // every frame
  ...
  void setupTextureImage () {
    ...
    // Submit a barrier to transition the image layout from transfer destionation optimal to shader
    // read-only optimal
  ...
  void setupDescriptorPool () {
    ...
    if (vkCreateDescriptorPool (device, &descriptorPoolCreateInfo, 0, &descriptorPool)
        != VK_SUCCESS) {
      vulkanAvailable = false;
      return;
    }
  ...
  void setupDescriptorSets () {
    ...
    if (vkAllocateDescriptorSets (device, &descriptorSetAllocateInfo, &descriptorSets[0])
        != VK_SUCCESS) {
  ...
  void setupCommandBuffers () {
    ...
    if (vkAllocateCommandBuffers (device, &commandBufferAllocateInfo, &commandBuffers[0])
        != VK_SUCCESS) {
      commandBuffers.clear ();
      vulkanAvailable = false;
      return;
  ...
  void setupDraw () {
    ...
    // Simultaneous use: this command buffer can be resubmitted to a queue before a previous
  ...
  void setupSemaphores () {
    ...
    for (std::size_t i = 0; i < maxFramesInFlight; i++) {
      ...
      if (vkCreateSemaphore (device, &semaphoreCreateInfo, 0, &imageAvailableSemaphores[i])
          != VK_SUCCESS) {
    ...
    for (std::size_t i = 0; i < maxFramesInFlight; i++) {
      ...
      if (vkCreateSemaphore (device, &semaphoreCreateInfo, 0, &renderFinishedSemaphores[i])
          != VK_SUCCESS) {
  ...
  void draw () {
    ...
    vkWaitForFences (
        device, 1, &fences[currentFrame], VK_TRUE, std::numeric_limits<uint64_t>::max ());
    ...
    // Wait for the swapchain image to be available in the color attachment stage before submitting
    // the queue
