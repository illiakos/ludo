...
public:
  ...
  std::shared_ptr<Tile> findTileByContextAndPosition(TileContext context,
                                                     int position);

  std::shared_ptr<PrefinishingTile>
  findPrefinishingTileByContextPositionAndTeam(TileContext context,
                                               int position, int teamId);
